...
extension ManagementInfoView {
    ...
    @IBAction func btnCaseCompletedClicked(_ sender: UIButton) {
        ...
        NetworkModule.shared.setCaseCompleted(status: "completed", completed_timestamp: strDate, onSuccess: { apiResponseCaseCompleted in
            
            print(apiResponseCaseCompleted)
            ...
            // Show Alert
            if success {
                self.delegate?.showAlert(withTitle: "CodeStrokeAlert", andMessage: "Case completed successfully", isSuccess: true)
            } else {
                self.delegate?.showAlert(withTitle: "CodeStrokeAlert", andMessage: "Error while submitting case", isSuccess: false)
            }
        })
    }
