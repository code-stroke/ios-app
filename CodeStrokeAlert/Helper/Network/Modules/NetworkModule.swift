...
extension NetworkModule {
    ...
    // SET Clinical Info
    func setClinicalInfo(pmhx: String, anticoags_last_dose: String, meds: String, anticoags: Bool, hopc: String, weight: Float, last_meal: String, onSuccess: @escaping (_ response: ApiResponseClinicalInfo) -> (),
                         onFailure: @escaping (_ reason: FailureReason) -> (),
                         onAction: @escaping (_ action: ResponseAction) -> (),
                         onError: @escaping (_ error: Error) -> (),
                         onComplete: ((_ success: Bool)->())? = nil) {
