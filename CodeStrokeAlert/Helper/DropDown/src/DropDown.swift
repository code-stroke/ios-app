...
public final class DropDown: UIView {
    ...
    fileprivate let DropDowntableView = UITableView()
    ...
    @objc fileprivate dynamic var tableViewBackgroundColor = DPDConstant.UI.BackgroundColor {
        willSet { DropDowntableView.backgroundColor = newValue }
    }
    ...
    @objc public dynamic var separatorColor = DPDConstant.UI.SeparatorColor {
        willSet { DropDowntableView.separatorColor = newValue }
        didSet { reloadAllComponents() }
    }
    ...
    @objc public dynamic var cornerRadius = DPDConstant.UI.CornerRadius {
        willSet {
            ...
            DropDowntableView.layer.cornerRadius = newValue
        }
        didSet { reloadAllComponents() }
    }
    ...
    public var cellNib = UINib(nibName: "DropDownCell", bundle: Bundle(for: DropDownCell.self)) {
        didSet {
            DropDowntableView.register(cellNib, forCellReuseIdentifier: DPDConstant.ReusableIdentifier.DropDownCell)
            templateCell = nil
            reloadAllComponents()
        }
    }
    ...
    fileprivate var minHeight: CGFloat {
        return DropDowntableView.rowHeight
    }
...
private extension DropDown {
    ...
    func setupUI() {
        ...
        DropDowntableView.backgroundColor = tableViewBackgroundColor
...
extension DropDown {
    ...
    public override func updateConstraints() {
        ...
        DropDowntableView.isScrollEnabled = layout.offscreenHeight > 0
    ...
    fileprivate func setupConstraints() {
        ...
        // Table view
        tableViewContainer.addSubview(DropDowntableView)
        DropDowntableView.translatesAutoresizingMaskIntoConstraints = false
        
        tableViewContainer.addUniversalConstraints(format: "|[tableView]|", views: ["tableView": DropDowntableView])
    }
...
extension DropDown {
    ...
    public func deselectRow(at index: Index?) {
        ...
        DropDowntableView.deselectRow(at: IndexPath(row: index, section: 0), animated: true)
    }
    
    /// Returns the index of the selected row.
    public var indexForSelectedRow: Index? {
        return (DropDowntableView.indexPathForSelectedRow as NSIndexPath?)?.row
    }
    
    /// Returns the selected item.
    public var selectedItem: String? {
        guard let row = (DropDowntableView.indexPathForSelectedRow as NSIndexPath?)?.row else { return nil }
